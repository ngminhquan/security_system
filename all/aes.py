...
def keySetupEnc(cipherKey):
    rk = [0] * 44
    rk[0] = struct.unpack('>I', cipherKey[0:4])[0]
    rk[1] = struct.unpack('>I', cipherKey[4:8])[0]
    rk[2] = struct.unpack('>I', cipherKey[8:12])[0]
    rk[3] = struct.unpack('>I', cipherKey[12:16])[0]
    i = 0
    r = 3
    while True:
        ...
        rk[r + 2] = rk[r - 2] ^ rk[r + 1]
        rk[r + 3] = rk[r - 1] ^ rk[r + 2]
        rk[r + 4] = rk[r] ^ rk[r + 3]
        if i == 9:
            break
        i += 1
...
key = long_to_bytes(0x0f1571c947d9e8590cb7add6af7f6798)
rk = keySetupEnc(key)
#Expand the cipher key into the decryption key schedule.

def keySetupDec(cipherKey):
    rk = keySetupEnc(cipherKey)
    Nr = 10
    i = 0
    j = Nr * 4
...
def encrypt(plaintext):
    s0 = struct.unpack('>I', plaintext[0:4])[0] ^ rk[0]
